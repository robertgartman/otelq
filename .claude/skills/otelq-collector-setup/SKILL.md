...
The canonical, authoritative instructions live in `.agents/skills/otelq-collector-setup/SKILL.md`. **Read that file and follow it exactly.** This Claude Code entry is only a pointer — all guidance is maintained in the canonical file so the two cannot drift.
