...
1. **Run the relevant test suite** created in Step 5. Prefer `just otelq-test` for the full suite; use a focused `uv run --with pytest --with "duckdb==1.5.5" pytest <path> -v` command while iterating when useful.
